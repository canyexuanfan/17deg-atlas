...
5. 每份文本材料由 Agent 逐文件完整阅读原文后生成准确的来源摘要、原子卡片与主题页候选，再调用：
   `workspace import-review --target <目录> --source-path <路径> --confirmation-id <该文件对应回执ID> --access ... --rights ... --origin-kind ... --authorship-status ... --intended-role ... --payload-file <本地UTF-8 JSON>`。
   责任字段必须与 `confirm-review` 完全一致；工具会逐项比对，不一致即拒绝写入。`import-review` 不得使用其他文件的 `confirmation_id`。
6. 知识类候选（`intended_role` 为 `knowledge` 或 `evidence`、`wiki-compilation=compile`）必须把生成文本写入本地忽略目录中的 UTF-8 JSON；字段固定为 `summary`、`card_question`、`card_answer`、`topic_names`、`evidence_quotes`。不得把中文摘要、卡片、主题和引文逐项拼接进 PowerShell 命令行，也不得同时使用 payload 文件与 `--summary/--card-question/--card-answer/--topic/--evidence`。引文每条至少 12 个字符、必须逐字出现在该文件原文中，且同时出现在摘要或卡片文本里；原文较长时至少 2 条，否则至少 1 条。禁止按文件名套通用模板或仅凭标题生成摘要；摘要、卡片、主题页必须基于该文件实际内容。工具会在写入任何 Raw/Wiki 对象前拒绝乱码、替代字符、空主题和无意义短文本。`wiki-compilation=raw-only` 时需附带 `--confirm-raw-only`，无需 payload。非文本材料先尝试提取文字；提取失败则保持待处理，不得强行生成摘要或卡片。
...
10. `workspace source-start` 返回 `ready-for-initial-sync` 后，使用同一目标和用户已选的 GitHub 仓库重新运行 `workspace start`，在得到对应确认后完成首次同步。同步成功后、向用户报告完成之前，必须立即运行 `workspace completion-audit --target <目录>`。只有返回 `terminal_state=complete` 才可宣布完成；返回 `needs-action` 时按 `issues` 逐项修复。`semantic-compilation-quality-failed` 表示乱码、责任字段不一致或主题未聚合，必须从保留的 Raw 重新编译，不能用对象数或 Git 同步掩盖；`runtime-recovery-residuals-remain` 必须展示残留并在保留或删除前取得用户选择。补审核、清理 inbox、原地址去留、runtime 残留、提交和同步完成后再次审计，直至通过。
