...
入口 Skill 默认安装在当前 Agent 支持的项目级隐藏 Skill 目录。不得把 `SKILL.md`、`skill.md`、`atlas.py`、`17deg-atlas.py`、克隆源码或其他安装辅助文件放到用户素材根目录、`questions` 或任何知识目录；Agent 没有标准项目级 Skill 目录时，应使用项目 `.17deg-atlas` 下的本地忽略目录作为安装来源，不能退回内容根目录。除非用户明确要求，不安装到全局 Skill 目录，也不把全局安装作为验收步骤。

首次安装或刷新 Skill 时，旧副本只能备份到 `.codex/backups/skills` 或项目 `.17deg-atlas/backups` 等本地忽略目录，不得备份到 `.codex/skills/_backups` 等 Agent 常规遍历可发现的位置。安装或刷新完成后必须验证同名入口在整个工作区只有一个，并在运行 `workspace plan` 前确认素材根目录没有安装辅助文件。工具识别出的成套、未跟踪官方安装残留应可逆隔离到 `.17deg-atlas/install-residuals`，不得当成用户知识，也不得按文件名排除用户真实的同名资料。
