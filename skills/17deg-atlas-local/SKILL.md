...
2. 向用户汇总网络安装、GitHub 登录、创建或连接仓库、首次同步等真实动作，并逐项取得必要确认。
3. 确认后使用 `scripts/atlas.py workspace start` 继续同一份计划，直到验证完成。

确认门只暂停对应真实动作，不改变用户目标。不得因为 GitHub 操作需要确认而静默改用 `knowledge agent-local-setup` 或其他纯本地流程；只有用户明确要求“仅保存在本机、不连接 GitHub”时才能切换为纯本地初始化。
