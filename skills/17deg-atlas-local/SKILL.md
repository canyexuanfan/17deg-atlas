...
9. 全部候选材料返回 `ready-for-initial-sync` 后，使用同一目标和用户已选的 GitHub 仓库重新运行 `workspace start`，在得到对应确认后完成首次同步。同步成功后、向用户报告完成之前，必须立即运行 `workspace completion-audit --target <目录>`。只有返回 `terminal_state=complete` 才可宣布完成；返回 `needs-action` 时按 `issues` 逐项修复（补审核、清理 runtime 安装残留、提交并同步 Git 等），再次同步并重新审计直至通过。
10. 完成审计后不得再写入源候选目录、迁移待整理区或目标 Git 工作树；如需补充整理，重新进入 `import-review` 流程、重新同步并再次审计。
